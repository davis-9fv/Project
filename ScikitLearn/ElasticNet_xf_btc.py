...
if supervised:
    ...
    X_train, X_test, y_train, y_test = train_test_split(raw_data[:, 0:4], raw_data[:, 4], test_size=0.20, shuffle=False)
else:
    series = read_csv('../Thesis/Bitcoin_historical_data_processed.csv', header=0, sep='\t')
    raw_data = series.values
    # Si se pone shuffle true, se mejora la respuesta
    X_train, X_test, y_train, y_test = train_test_split(raw_data[:, 0:3], raw_data[:, 3], test_size=0.20, shuffle=True)
